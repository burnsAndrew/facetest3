...
async function drawing_faces(overlay, fullFaceDescriptions) {
  ...
  faceapi.draw.drawFaceLandmarks(overlay, fullFaceDescriptions, {
    lineWidth: 4,
    color: "red"
  });
}
async function screenshot_save(input, canvas) {
  // Making effects
  $("#send")
    .text("Sending 3 photos for training model...")
    .attr("disabled");
  ...
  savingPhoto({ label: label, photo: dataURI, displaySize: displaySize }).then(
    function(result) {
      // Read result of the Cloud Function.
      var data = result.data;
